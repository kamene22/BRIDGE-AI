...
|---|---|---|---|
| **LLM** | Gemini 3.1 Flash-Lite (Text RAG) & Gemini 3.1 Flash Live (Voice) | GPT-4o, Claude 3.5 Haiku | Strong instruction-following, native low-latency WebSocket audio streaming, and low latency at a cost profile that suits production deployment. |
...
| **Embeddings** | Gemini Embeddings (Embedding 2) | OpenAI `text-embedding-3-large`, Voyage AI | Keeps the pipeline within a single ecosystem for the PoC, minimizing API surface area to manage under a tight timeline. |
| **Vector database** | ChromaDB Dual-Index | Pinecone, Weaviate, a managed vector service | For a small, static, nine-document corpus, a managed cloud vector service adds operational overhead and cost with no real benefit — there's no scale or update-frequency problem here that justifies it. A local, lightweight store with multi-index isolation is the right-sized tool for this stage. |
| **Backend** | FastAPI | Flask, Django | Lightweight orchestration of retrieval, guardrails, prompting, and logging without unnecessary framework weight. Deployed live on Render. |
| **Frontend (demo)** | Modern React 18 + Vite + TypeScript Web UI (`bridge_ai_ui/`) featuring real-time Voice Lounge & Text Chat Panel | Streamlit local app | Full production-grade web application featuring Gemini Live WebSocket BidiGenerateContent voice mentoring, real-time transcription, animated voice orb, and ephemeral token security (`/api/token`). Deployed live on Vercel (`https://bridge-ai-eosin.vercel.app`). |
